...
logging.basicConfig(filename=os.path.join(os.getcwd(), 'transport_analyse.log'),
                                          level=logging.DEBUG,
                                          filemode='a',
...
amap = Amap(configs['amap']['key'], '021', logger=logger)
    ...
    try:
        ...
        for poi in poi_infos:
            ...
            if route_result['status'] != '1' or route_result['infocode'] != '10000':
                logger.error(f"amap transit direction result error: infocode: {route_result['infocode']}; status: {route_result['status']}, info:{route_result['info']}")
                raise Exception("amap transit direction result error")
